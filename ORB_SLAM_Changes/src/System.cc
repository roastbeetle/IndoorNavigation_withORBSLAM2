...
namespace ORB_SLAM2
{
	...
	System::System(const string &strVocFile, const string &strSettingsFile, const string &strMapFile, const eSensor sensor,
		const bool bUseViewer,const bool onlyloc, const bool load_map) :mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false), mbActivateLocalizationMode(false),
		mbDeactivateLocalizationMode(false)
	{
	    ...
	    string strPathMap = cwd + "/"+strMapFile.c_str();
	    ...
	    cout << "use viewer : " << bUseViewer << "  , only localization : " << onlyloc << endl;  
