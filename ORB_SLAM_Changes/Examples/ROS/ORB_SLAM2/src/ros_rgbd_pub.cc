...
bool pub_all_pts = true; // pub_all_pts once when ORB-SLAM startup
...
int main(int argc, char **argv)
{
    ...
    string view_s(argv[4]);
    string onlylo_s(argv[5]);
    string loadmap_s(argv[6]);
    if (view_s == "true"){
	    view = true;
    }
    if (onlylo_s == "true"){
    ...
    cout << "onlylo"<< onlylo <<endl;
    // Create SLAM system. It initializes all system threads and gets ready to process frames.
    ORB_SLAM2::System SLAM(argv[1],argv[2],argv[3],ORB_SLAM2::System::RGBD,view,onlylo,loadmap);
    ...
    message_filters::Subscriber<sensor_msgs::Image> rgb_sub(nh, "/camera/color/image_raw_throttle", 1);
    message_filters::Subscriber<sensor_msgs::Image> depth_sub(nh, "/camera/aligned_depth_to_color/image_raw_throttle", 1);
	...
	ros::spinOnce();
	cout << "publish .. ";
	publish(SLAM, pub_pts_and_pose, pub_all_kf_and_pts, pub_cur_camera_pose, 1);
	cout << "over" << endl;
	ros::spinOnce();
			 ...
			 ros::Publisher &pub_all_kf_and_pts, ros::Publisher &pub_cur_camera_pose, int frame_id)
{
    ...
    //static bool pub_all_pts = true; // pub_all_pts once when ORB-SLAM startup
