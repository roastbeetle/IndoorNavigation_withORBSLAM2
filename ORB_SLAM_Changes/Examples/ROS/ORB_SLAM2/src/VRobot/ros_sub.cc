...
AStar::Vec2i g_startPoitn{0, 0}, g_targetPoint{0, 0}; //起点{x, y}，终点{x, y}
...
int main(int argc, char **argv)
{
	...
	generator.setWorldSize({w, h});						 //map Size
	generator.setHeuristic(AStar::Heuristic::manhattan); //启发式：欧几里得 sqrt(x^2 + y^2)
	generator.setDiagonalMovement(true);				 //true:8个方向 false:4个方向

	//thread pathPlan(findpath_thread);
	...
	*/
	...
	/* {
		...
		for (auto coordinate : path)
		{
			...
			dst.at<uchar>(coordinate.x, coordinate.y) = 55;
		}
...

	{
		...
		{
			std::lock_guard<std::mutex> lock(g_gridMapMutex);
			dst = grid_map_thresh.clone();
		}
		cv::circle(dst, cv::Point(g_camera_pos_grid_x * resize_factor, (g_camera_pos_grid_z)*resize_factor), 2 * resize_factor, cv::Scalar(50));
		cv::line(dst, cv::Point(g_camera_pos_grid_x * resize_factor, (g_camera_pos_grid_z)*resize_factor),
				 cv::Point(g_camera_ori_grid_x * resize_factor, (g_camera_ori_grid_z)*resize_factor), cv::Scalar(60), 1, 8, 0);

		cv::circle(dst, cv::Point(g_target_x * resize_factor, (g_target_z)*resize_factor), 2 * resize_factor, cv::Scalar(0), cv::FILLED);
		...
		cv::Mat dst2;
		cv::flip(dst, dst2, 0);
		cv::imshow("grid_map_thresh", dst2);
	}
...
void findpath_thread(void)
{
	...
	while (g_findpathRunning)
	{
		while (g_target_is_set)
		{
			g_target_is_set = false;

			g_startPoitn = {g_camera_pos_grid_z, g_camera_pos_grid_x};
			g_targetPoint = {g_target_z, g_target_x};

			{
				std::lock_guard<std::mutex> lock(g_gridMapMutex);
				//腐蚀
				cv::erode(grid_map_thresh, grid_map, element);
				//cv::imshow("grid_map",grid_map);
			}
			...
			for (int row = 0; row < h; ++row)
			{
				for (int col = 0; col < w; ++col)
				{
					auto c = grid_map.at<uchar>(col, row);
					if (c != 255) //Collision
						generator.addCollision({col, row});
				}
			}

			time_t t1 = time(NULL);
			std::cout << "Generate path ... " << t1 << endl;
			path = generator.findPath(g_startPoitn, g_targetPoint); //寻路
			std::cout << "Generate over. time:" << time(NULL) - t1 << endl;
		}
		usleep(100000);
	}
}
